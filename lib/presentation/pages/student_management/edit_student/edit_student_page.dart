...
import 'package:template/core/helper/izi_date.dart';
import 'package:template/core/helper/izi_size_util.dart';
import 'package:template/core/utils/color_resources.dart';
import 'package:template/core/utils/images_path.dart';
import 'package:template/presentation/pages/student_management/edit_student/edit_student_controller.dart';
...
class EditStudentPage extends GetView<EditStudentController> {
          ...
          children: [
            Padding(
              padding: const EdgeInsets.only(
                  top: 40, left: 8, right: 22, bottom: 20),
              child: Stack(
                children: [
                  WidgetSearch(),
                  const WidgetHeader(),
  ...
  Widget buildAllInputText() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        buildTextInput('full_name'.tr, controller.studentModel.fullName, null,
            controller.fullnameController),
        Row(
          children: [
            buildTextInput('class'.tr, controller.studentModel.classInfo, 175,
                controller.classController),
            const Spacer(),
            buildTextInput('student_id'.tr, controller.studentModel.id, 131,
        ...
        Row(
          children: [
            buildTextInput(
                'email'.tr,
                controller.studentModel.contactInfo.email,
                199,
                controller.emailController),
            const Spacer(),
            buildTextInput(
                'date_of_birth'.tr,
                IZIDate.formatDateFromMilliseconds(
                    controller.studentModel.dateOfBirth ?? 0),
                110,
        ...
        Row(
          children: [
            buildTextInput(
                'address'.tr,
                controller.studentModel.contactInfo.address,
                191,
                controller.addressController),
            const Spacer(),
            buildTextInput(
                'phone_number'.tr,
                controller.studentModel.contactInfo.phoneNumber,
                117,
        ...
        buildTextInput(
            'avarage_mark'.tr,
            controller.studentModel.averageScore.toString(),
            null,
  ...
  Widget buildTextInput(String? tittle, String? text, double? width,
